...
* @version $Id: admin_actions.php 39818 2014-05-13 13:15:13Z weinert $
*/
...
class admin_actions extends base_actions {
  ...
  */
  function saveGroup() {
    ...
    if (!PapayaFilterFactory::isNotXml($this->params['group_name'], TRUE)) {
  ...
  */
  function saveAction() {
    ...
    if (!PapayaFilterFactory::isNotXml($this->params['action_name'], TRUE)) {
  ...
  */
  function saveObserver() {
    ...
    if (!PapayaFilterFactory::isGuid($this->params['observer_guid'])) {
  ...
  */
  function importXML() {
    ...
    $dom = PapayaXmlDocument::createFromXML(file_get_contents($tempFileName));
    if (!($dom && isset($dom->documentElement))) {
    ...
    $config = array();
    foreach ($dom->xpath()->evaluate('/action-observers/action-group') as $groupNode) {
      $groupName = $dom->xpath()->evaluate('string(name)', $groupNode);
      $actions = array();
      /** @var PapayaXmlElement $actionNode */
      foreach ($dom->xpath()->evaluate('action', $groupNode) as $actionNode) {
        $actionName = $actionNode->getAttribute('name');
        $observers = array();
        /** @var PapayaXmlElement $observerNode */
        foreach ($dom->xpath()->evaluate('observer', $actionNode) as $observerNode) {
